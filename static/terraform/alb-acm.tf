...
# Second wildcard certificate for riv subdomain
resource "aws_acm_certificate" "wildcard_riv" {
  count                     = var.domain != "" ? 1 : 0
  domain_name               = "*.riv.${var.domain}"
  validation_method         = "DNS"
  subject_alternative_names = ["riv.${var.domain}"]
  lifecycle {
    create_before_destroy = true
  }
}

resource "aws_route53_record" "validation_riv" {
  for_each = var.domain != "" ? {
    for dvo in aws_acm_certificate.wildcard_riv[0].domain_validation_options : dvo.domain_name => {
      name   = dvo.resource_record_name
      record = dvo.resource_record_value
      type   = dvo.resource_record_type
    }
...
resource "aws_acm_certificate_validation" "wildcard_riv" {
  count                   = var.domain != "" ? 1 : 0
  certificate_arn         = aws_acm_certificate.wildcard_riv[0].arn
  validation_record_fqdns = [for record in aws_route53_record.validation_riv : record.fqdn]
}
