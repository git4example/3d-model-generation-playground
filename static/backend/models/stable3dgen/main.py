...
from inference import Stable3DGenInference
...
async def lifespan(app: FastAPI):
    ...
    try:
        logger.info("Loading Stable3DGen models...")
        inference_model = Stable3DGenInference()
        logger.info("Stable3DGen models loaded successfully")
    ...
    logger.info("Shutting down Stable3DGen service...")
...
app = FastAPI(title="Stable3DGen Model Service", lifespan=lifespan)
...
async def process_job_async(job_id: str, request: dict):
    ...
    try:
        ...
        ss_guidance_strength = request.get('ss_guidance_strength', 3.0)
        ss_sampling_steps = request.get('ss_sampling_steps', 50)
        slat_guidance_strength = request.get('slat_guidance_strength', 3.0)
        slat_sampling_steps = request.get('slat_sampling_steps', 6)
        output_format = request.get('model_save_format', 'glb')

        logger.info(f"Job {job_id}: Processing - remove_bg: {remove_background}, seed: {seed}")
...
async def generate_3d(request: dict, background_tasks: BackgroundTasks):
    ...
    job = job_manager.create_job(job_id, 'stable3dgen', request.get('input_type', 'image'))
...
async def generate_3d_from_s3(request: dict, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        job_id = str(uuid.uuid4())
        job = job_manager.create_job(job_id, 'stable3dgen', 'image')
...
async def model_info():
    ...
    return {
        "name": "Stable3DGen",
        "description": "High-fidelity 3D geometry generation from images via normal bridging",
        "paper": "Hi3DGen: High-fidelity 3D Geometry Generation from Images via Normal Bridging",
        "capabilities": ["image-to-3d", "normal-bridging", "two-stage-generation"],
        "output_formats": ["glb", "obj", "ply", "stl"],
        "gpu_required": True,
        "recommended_gpu": "A10G (24GB VRAM)",
        "vram_requirement": "20GB+",
        "inference_speed": "~2-3 minutes",
        "async_mode": True,
        "s3_enabled": s3_uploader.enabled,
        "s3_required": True,
        ...
        "features": {
            ...
            "normal_estimation": "StableNormal",
            "sparse_structure_generation": "Configurable steps and guidance",
            "structured_latent_generation": "Configurable steps and guidance"
        }
    }
...
async def test_generate():
    ...
    try:
        ...
        logger.info(f"Test generation with cat.jpg: remove_bg={remove_background}, seed={seed}")
        ...
        return {
            ...
            "output": {
                "glb_base64": base64.b64encode(result["mesh_data"]).decode('utf-8'),
                "vertices": result["vertices"],
                "faces": result["faces"],
...
async def root():
    """Root endpoint"""
    return {
        "service": "Stable3DGen 3D Generation API",
        "status": "running",
        "endpoints": {
